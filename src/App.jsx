...
const PROPERTIES = [
  { id: "treetop", loc: "SM", name: "Light-Filled Tree-Top Getaway", type: "Guesthouse 1BR", area: "Santa Monica",
    ...
    colors: ["#87CEEB", "#D4B896", "#8FBC8F", "#B8A898"] },
  { id: "mcm", loc: "SM", name: "MCM Studio with Patio", type: "Guesthouse 1BR", area: "Santa Monica",
    ...
    colors: ["#A8B89C", "#C4B898", "#98ACC4", "#B8C4A0"] },
  { id: "lbup", loc: "LB", name: "Beach Oasis - 2BR", type: "Apartment 2BR", area: "Belmont Heights, Long Beach",
    ...
    colors: ["#B8A088", "#A8B8C8", "#C8B8A0", "#98A888"] },
  { id: "lbdn", loc: "LB", name: "Beach Getaway Studio", type: "Studio", area: "Belmont Heights, Long Beach",
    ...
    colors: ["#A0A8B8", "#B8B0A0", "#A8C0A8", "#C0B0A8"] },
  { id: "silver", loc: "SL", name: "Skyline Views - Silver Lake", type: "Studio", area: "Silver Lake, LA",
    ...
    colors: ["#7BA4C4", "#C4B898", "#A8B498", "#B8A8A0"] },
];
...
function makeBookedSet(propertyId) {
  ...
  for (var i = 0; i < propertyId.length; i++) {
    seed = seed + propertyId.charCodeAt(i);
  }
  var now = new Date();
  for (var j = 0; j < 90; j++) {
...
function StarIcon() {
  return (
    <svg width="12" height="12" viewBox="0 0 20 20" fill="currentColor">
      <path d="M10 1l2.39 4.84L18 6.71l-4 3.9.94 5.51L10 13.49l-4.94 2.63L6 10.61l-4-3.9 5.61-.87z" />
    </svg>
  );
}
...
function Calendar(props) {
  ...
  var monthState = useState(initialMonth);
  var viewMonth = monthState[0];
  var setViewMonth = monthState[1];
  var yearState = useState(initialYear);
  var viewYear = yearState[0];
  var setViewYear = yearState[1];
  ...
  for (var d = 1; d <= daysInMonth; d++) {
    ...
    var isCheckin = dateStr === checkin;
    var isCheckout = dateStr === checkout;
    var isInRange = false;
  ...
  function handleDayClick(dateStr) {
    if (!checkin || (checkin && checkout)) {
      ...
      var hasBlock = false;
      while (cur < end) {
        var ck = cur.getFullYear() + "-" + String(cur.getMonth() + 1).padStart(2, "0") + "-" + String(cur.getDate()).padStart(2, "0");
        if (booked[ck]) { hasBlock = true; break; }
        cur.setDate(cur.getDate() + 1);
      }
      if (hasBlock) {
        onSelect(dateStr, null);
  ...
  function getCellStyle(cell) {
    if (cell.isCheckin || cell.isCheckout) {
      return { background: "#1a1a1a", color: "#FAF7F2", fontWeight: 700, borderRadius: cell.isCheckin ? "4px 0 0 4px" : "0 4px 4px 0" };
    }
    if (cell.isInRange) {
      return { background: "rgba(58,107,138,0.15)", color: "#3a6b8a" };
    }
    if (cell.isAvailable) {
      return { background: "rgba(139,174,196,0.08)", color: "#3a6b8a", cursor: "pointer" };
    }
    if (cell.isBooked && !cell.isPast) {
      return { background: "rgba(196,139,139,0.1)", color: "#b08080" };
    }
    return { color: "#ccc" };
  }
    ...
    <div style={{ marginTop: 12 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(7,1fr)", gap: 2, textAlign: "center" }}>
        {DAY_LABELS.map(function (label, idx) {
          return <div key={"lbl-" + idx} style={{ fontSize: 9, color: "#8a8278", fontWeight: 600, padding: 2 }}>{label}</div>;
        })}
        {cells.map(function (cell) {
          ...
          return (
            <div key={cell.key}
              onClick={function () { if (cell.isAvailable) handleDayClick(cell.dateStr); }}
              style={{ fontSize: 11, fontWeight: style.fontWeight || 500, padding: "6px 0", borderRadius: style.borderRadius || 3,
                background: style.background || "transparent", color: style.color || "#ccc", cursor: style.cursor || "default" }}>
              {cell.day}
            </div>
          );
        })}
      ...
      <div style={{ display: "flex", gap: 10, marginTop: 6, justifyContent: "center", fontSize: 9, color: "#8a8278" }}>
        <span>
          <span style={{ display: "inline-block", width: 7, height: 7, borderRadius: 2, background: "rgba(139,174,196,0.2)", marginRight: 3, verticalAlign: "middle" }} />
          Available
        </span>
        <span>
          <span style={{ display: "inline-block", width: 7, height: 7, borderRadius: 2, background: "rgba(196,139,139,0.2)", marginRight: 3, verticalAlign: "middle" }} />
          Booked
        </span>
        <span>
          <span style={{ display: "inline-block", width: 7, height: 7, borderRadius: 2, background: "#1a1a1a", marginRight: 3, verticalAlign: "middle" }} />
          Selected
        </span>
      </div>
      {checkin && (
        <div style={{ textAlign: "center", marginTop: 6, fontSize: 11, color: checkout ? "#3a6b8a" : "#8a8278", fontWeight: 500 }}>
          {checkout ? checkin + "  to  " + checkout : "Now select check-out date"}
        </div>
      )}
    </div>
  );
...
function PropertyCard(props) {
  ...
  var ciState = useState(null);
  var checkin = ciState[0];
  var setCheckin = ciState[1];

  var coState = useState(null);
  var checkout = coState[0];
  var setCheckout = coState[1];

  var photoState = useState(0);
  var activePhoto = photoState[0];
  var setActivePhoto = photoState[1];

  var isMonthly = property.stay === "monthly";
  var pricing = !isMonthly ? calcPrice(property, checkin, checkout) : null;

  function handleDateSelect(ci, co) {
    setCheckin(ci);
    setCheckout(co);
  }
  ...
  return (
    <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 32, marginBottom: 56 }} className="propgrid">
      <div>
        <div style={{ width: "100%", aspectRatio: "4/3", borderRadius: 4, background: property.colors[activePhoto], marginBottom: 6 }} />
        <div style={{ display: "flex", gap: 4 }}>
          {property.colors.map(function (c, i) {
            return (
              <div key={i} onClick={function () { setActivePhoto(i); }}
                style={{ flex: 1, aspectRatio: "1", borderRadius: 3, background: c, cursor: "pointer",
                  opacity: i === activePhoto ? 1 : 0.4, outline: i === activePhoto ? "2px solid #1a1a1a" : "none", outlineOffset: 1 }} />
            );
          })}
        </div>
      </div>
      <div>
        ...
        <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 8 }}>
          <span style={{ color: "#c4952a", display: "flex", alignItems: "center", gap: 3 }}>
            <StarIcon /><b style={{ color: "#1a1a1a", fontSize: 13 }}>{property.rate}</b>
          </span>
          <span style={{ fontSize: 12, color: "#8a8278" }}>{property.revs} reviews</span>
        </div>
        {isMonthly ? (
          <div style={{ marginBottom: 8 }}>
            <span style={{ fontFamily: "'Playfair Display',serif", fontSize: 21, fontWeight: 700 }}>{"$" + property.monthly.toLocaleString()}</span>
            <span style={{ fontSize: 12, color: "#8a8278", marginLeft: 4 }}>/ month</span>
          </div>
        ) : (
        ...
        {pricing && (
          <div style={{ background: "rgba(139,174,196,0.06)", border: "1px solid rgba(139,174,196,0.15)", padding: 12, marginTop: 6, marginBottom: 6, fontSize: 12 }}>
            <div style={{ display: "flex", justifyContent: "space-between", color: "#4a4a4a", marginBottom: 3 }}>
              <span>{"$" + property.price + " x " + pricing.nights + " nights"}</span><span>{"$" + pricing.sub}</span>
            </div>
            {pricing.cl > 0 && (
              <div style={{ display: "flex", justifyContent: "space-between", color: "#4a4a4a", marginBottom: 3 }}>
                <span>Cleaning</span><span>{"$" + pricing.cl}</span>
              </div>
            )}
            <div style={{ display: "flex", justifyContent: "space-between", color: "#4a4a4a", marginBottom: 3 }}>
              <span>{"Tax (" + Math.round(property.tax * 100) + "%)"}</span><span>{"$" + pricing.tot}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", color: "#3a6b8a", marginBottom: 3 }}>
              <span>Airbnb fee</span><span>{"$0 (save $" + pricing.saved + ")"}</span>
            </div>
            <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 700, fontSize: 14, borderTop: "1px solid rgba(0,0,0,0.1)", paddingTop: 6, marginTop: 3 }}>
              <span>Total</span><span>{"$" + pricing.total}</span>
            </div>
          </div>
        )}
        {!isMonthly && (
          <Calendar propertyId={property.id} checkin={checkin} checkout={checkout} onSelect={handleDateSelect} />
        )}
        <button onClick={function () { onBook(property, checkin, checkout); }}
          style={{ marginTop: 14, fontFamily: "'DM Sans',sans-serif", fontSize: 11, fontWeight: 600, letterSpacing: "0.08em",
            textTransform: "uppercase", padding: "11px 22px", background: "#1a1a1a", color: "#FAF7F2", border: "none", cursor: "pointer" }}>
...
function BookingForm(props) {
  ...
  var initCI = props.initCI;
  var initCO = props.initCO;
  ...
  useEffect(function () {
    if (selected) {
      setForm(function (prev) {
        return { name: prev.name, email: prev.email, property: selected.id, checkin: initCI || prev.checkin, checkout: initCO || prev.checkout, agree: prev.agree };
      });
    }
  }, [selected, initCI, initCO]);
  ...
  if (done) {
    return (
      <div style={{ textAlign: "center", padding: 40 }}>
        ...
        <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: 22, fontWeight: 700, marginTop: 10 }}>
          {isMonthly ? "Inquiry Sent!" : "Confirmed!"}
        </h3>
  ...
  return (
    <div>
      ...
      {pricing && (
        <div style={{ background: "rgba(139,174,196,0.06)", border: "1px solid rgba(139,174,196,0.15)", padding: 14, marginBottom: 14, fontSize: 13, display: "flex", justifyContent: "space-between", fontWeight: 700 }}>
          <span>Total</span><span>{"$" + pricing.total}</span>
        </div>
      )}
      ...
      <button onClick={function () { if (form.name && form.email && form.property && form.agree) setDone(true); }}
        style={{ width: "100%", fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 600, letterSpacing: "0.08em",
          textTransform: "uppercase", padding: "14px 24px", background: "#1a1a1a", color: "#FAF7F2", border: "none", cursor: "pointer" }}>
...
export default function App() {
  ...
  var spState = useState(null);
  var selectedProp = spState[0];
  var setSelectedProp = spState[1];

  var ciState = useState("");
  var bookCI = ciState[0];
  var setBookCI = ciState[1];

  var coState = useState("");
  var bookCO = coState[0];
  var setBookCO = coState[1];
  ...
  function handleBook(prop, ci, co) {
  ...
  return (
    <div style={{ background: "#FAF7F2", minHeight: "100vh", fontFamily: "'DM Sans', sans-serif" }}>
      ...
      <div style={{ position: "fixed", top: 0, left: 0, right: 0, zIndex: 100, background: "rgba(250,247,242,0.95)", backdropFilter: "blur(12px)",
        borderBottom: "1px solid rgba(0,0,0,0.06)", padding: "12px 20px", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
      ...
      <section id="hero" style={{ minHeight: "100vh", display: "flex", flexDirection: "column", justifyContent: "center",
        alignItems: "center", textAlign: "center", padding: "100px 20px 60px", background: "linear-gradient(180deg,#FAF7F2,#F0EBE3 40%,#D4CDC2)" }}>
        <p style={{ fontSize: 10, fontWeight: 600, letterSpacing: "0.2em", textTransform: "uppercase", color: "#8a8278", marginBottom: 18 }}>
          Superhost | 275+ Reviews | 4.86 | 13 Years
        </p>
        ...
        <p style={{ fontSize: 13, color: "#a09888", maxWidth: 380, marginBottom: 32, lineHeight: 1.7 }}>
          Santa Monica, Long Beach, and Silver Lake. Book directly and skip the Airbnb fees.
        </p>
        <button onClick={function () { scrollTo("properties"); }}
          style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, fontWeight: 600, letterSpacing: "0.08em",
            textTransform: "uppercase", padding: "14px 28px", background: "#1a1a1a", color: "#FAF7F2", border: "none", cursor: "pointer" }}>
          Browse Properties
        </button>
      </section>
      ...
      <section id="properties" style={{ padding: "70px 20px", maxWidth: 1000, margin: "0 auto" }}>
        ...
        <div style={{ display: "flex", justifyContent: "center", gap: 6, margin: "16px 0 36px", flexWrap: "wrap" }}>
          {LOCATIONS.map(function (loc) {
            return (
              <button key={loc.id} onClick={function () { setLocFilter(loc.id); }}
                style={{ fontFamily: "'DM Sans',sans-serif", fontSize: 12, padding: "7px 16px", cursor: "pointer",
                  border: locFilter === loc.id ? "1.5px solid #1a1a1a" : "1.5px solid rgba(0,0,0,0.1)",
                  background: locFilter === loc.id ? "#1a1a1a" : "transparent",
                  color: locFilter === loc.id ? "#FAF7F2" : "#8a8278" }}>
                {loc.label}
              </button>
            );
          })}
        ...
        {filtered.map(function (p) {
          return <PropertyCard key={p.id} property={p} onBook={handleBook} />;
        })}
      </section>
